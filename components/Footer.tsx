...
const FooterLinks = ({heading,items} : any) => (
    <div className="flex-1 justify-start items-start">
        <h3 className="text-xl font-poppins font-semibold mb-10 dark:text-white text-nft-black-1 ">{heading}</h3>
        {items.map((item : string,index : number) => (
            <p
                key={index}
                className="text-base font-poppins text-nft-black-1 dark:text-white font-normal cursor-pointer dark:hover:text-nft-gray-1 hover:text-nft-black-1 my-3"
            >
                {item}
            </p>
        ))}
                    ...
                    <div className="flex flex-row sm:mt-4">
                        {[images.instagram,images.twitter,images.telegram,images.discord].map((image,index) => (
                            <div key={index} className={`mx-2 cursor-pointer`}>
                                <Image src={image} alt="social media icon" width={24} height={24} className={`${theme === 'light' && 'filter invert'}`}/>
                            </div>
                        ))}
                    </div>
                </div>
